...
欢迎来到 EverOS。加入我们的社区，一起改进项目，并与来自世界各地的优秀开发者协作。
...
**EverOS** 不只让 AI 记住发生过什么，还让它理解这些记忆背后的意义，并将其用于当前决策。EverOS 在 LoCoMo 基准测试上实现了 **93% 推理准确率**，通过结构化提取、智能检索与持续演化的画像构建，为对话式 AI Agent 提供长期记忆能力。

![EverOS Architecture Overview][overview-image]

**工作方式：** EverOS 从对话中提取结构化记忆（Encoding），将其整理为情景记忆和画像（Consolidation），并在需要时智能检索相关上下文（Retrieval）。
...
EverOS 在 LoCoMo 基准测试上实现了 **93% 总体准确率**，表现优于可比记忆系统。
...
EverOS 支持使用 [GitHub Codespaces][codespaces-link] 进行云端开发。这意味着你不需要在本地手动配置 Docker、网络环境或兼容性问题。
...
EverOS 已接入以下 AI 驱动问答平台。它们可以帮助你快速、准确地获取答案，并支持从基础安装到高级实现的多语言问题查询。
...
我们热爱开源社区的活力。无论你是在修 Bug、做功能、完善文档，还是提出一些大胆的新想法，每一个 PR 都会推动 EverOS 往前走。欢迎查看 [Issues][issues-link] 寻找适合你的切入点，然后把你的成果提交给我们。一起构建记忆系统的未来。
...
> **欢迎各种形式的贡献** 🎉
>
> 一起把 EverOS 做得更好。从代码到文档，每一份贡献都很重要。也欢迎你把自己的项目分享到社交平台，激发更多人的灵感。
>
> 欢迎通过 𝕏 联系 EverOS 维护者 [@elliotchen200][elliot-x-link]，或通过 GitHub 联系 [@cyfyifanchen][cyfyifanchen-link]，获取项目动态、参与讨论并展开合作。
